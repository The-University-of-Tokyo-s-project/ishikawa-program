...
from std_msgs.msg import Bool#for enable_motor tpic
...
class AvoidPredictedAccident:
	...
	enable_steer_motor = Bool()#add for stearing 11/2 
	enable_steer_motor.data = False
			...
			for i in range(0,data_num):
				if msg.max_distance[i] > AVOID_THRESHOLD:
					if abs( steer0deg_index - i ) < abs( steer0deg_index - avoid_index ):
						avoid_index = i
			if avoid_index == -1:#there in not any direction which is safe. 11/2
				self.seniorcar_command.accel_opening = 0
				#self.seniorcar_command.steer_angle   = 0
				self.enable_steer_motor.data = False#add for stearing off. 11/2
			else:#there are same direction which is safe. 11/2
				self.seniorcar_command.accel_opening = 90
				self.enable_steer_motor.data = True#add for stearig on. 11/2
