...
/*
const float P_R_Z = 0.00001;		// 反射したという情報が返ってきた時にそのグリッドが路面である確率 タイヤ高さ推定0.06、走行可能領域検出では0.09くらいがなんかよさそう
const float P_R_notZ = 0.000001;		// 通過したという情報が返ってきた時にそのグリッドが路面である確率
const float P_R_DEFAULT = 0.000005;	// 何も情報が得られていないときにそのグリッドが路面である確率
*/

const float P_R_Z = 0.06;		// 反射したという情報が返ってきた時にそのグリッドが路面である確率 タイヤ高さ推定0.06、走行可能領域検出では0.09くらいがなんかよさそう
