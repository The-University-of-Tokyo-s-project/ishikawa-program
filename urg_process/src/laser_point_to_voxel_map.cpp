...
void PointCloudCallback(const sensor_msgs::PointCloud::ConstPtr& msg){
  ...
  try{
      ...
      //listener.waitForTransform("/odom", "/laser_tilt", now, ros::Duration(1.0));
      //listener.lookupTransform("/odom", "/laser_tilt", now, transform);
      listener->waitForTransform("/odom","/laser_tilt", ros::Time(0), ros::Duration(1.0));
      listener->lookupTransform("/odom","/laser_tilt", ros::Time(0), transform);
  }
...
void moveMapCenter(){
  ...
  try{
    ...
    //ros::Time now = ros::Time::now();
    //listener.waitForTransform("/odom", "/map_center", now, ros::Duration(1.0));
    //listener.lookupTransform("/odom", "/map_canter", now, transform);
    listener->waitForTransform("/odom","/map_center", ros::Time(0), ros::Duration(1.0));
    listener->lookupTransform("/odom","/map_center", ros::Time(0), transform);
  }
  catch (tf::TransformException ex){
    ROS_ERROR("%s",ex.what());
