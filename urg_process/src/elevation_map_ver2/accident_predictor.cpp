...
void AccidentPredictor::generatePath(float pos_x,float pos_y,float yaw,float vehicle_velocity)/*k I will arrange this part. 11_8_10:51 */
{
	int X = 0;
	int Y = 1;
	int TH = 2;
...
bool AccidentPredictor::isCollisionByTirePos(CalculatedVehicleState state_now,CalculatedVehicleState state_old){
	...
	return ( abs(state_now.tire_pos[0][2] - state_old.tire_pos[0][2]) > SENIORCAR_DRIVABLE_STEP_HEIGHT || abs(state_now.tire_pos[1][2] - state_old.tire_pos[1][2]) > SENIORCAR_DRIVABLE_STEP_HEIGHT );
};
...
void AccidentPredictor::printPredictedFrontLeftTireHeight(){
	...
	cout << state.tire_pos[1][0] << "," << state.tire_pos[1][1] << "," << state.tire_pos[1][2] + SENIORCAR_WHEEL_RADIUS << endl;
}
