...
APPRORCH_THRESHOLD = 1.5  # 何mまで近づいたら次の点を指示するか//20180325
...
class playRecordedWaypoint:
    ...
    def callback(self,data):
        ...
        if len(self.recorded_waypoints) - 1 > self.index:
            if self.calc_distance_between_poses( self.recorded_waypoints[self.index] , pose_info ) < APPRORCH_THRESHOLD:
                ...
                #pub_pose.header.stamp = rospy.Time(0)
    ...
    def sendGoalToServer(self):
        ...
        #self.goal.target_pose.header.stamp = rospy.Time(0)
    ...
    def get_waypoints_from_txt(self): 
        ...
        filename = rospy.get_param('waypoint_file_path',"/home/ishikawa/waypoint_020.txt")
        ...
        for i in f.readlines():
            ...
            '''
            pose_info.position.x    = float(i[0])
            pose_info.position.y    = 0.0#float(i[1])
            pose_info.position.z    = 0.0#float(i[2])
            pose_info.orientation.x = 0.0#float(i[3])
            pose_info.orientation.y = 0.0#float(i[4])
            pose_info.orientation.z = 0.0#float(i[5])
            pose_info.orientation.w = 1.0#float(i[6])
            '''
